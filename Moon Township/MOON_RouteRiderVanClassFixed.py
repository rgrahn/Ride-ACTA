...
class rider(object):
    def __init__(self, rideID, origTime, orig, dest, timePeriod, USE_orig_lat, 
    ...
    def convertTimeToSec(self, timeStr):
        splitTimes = timeStr.split(':')
        timeSeconds = int(splitTimes[0])*3600 + int(splitTimes[1])*60 + int(splitTimes[2])
        return timeSeconds
...
class van(object):
    ...
    def getTripInfo(self, orig, dest, graph):
        route = nx.shortest_path(graph, source = orig, target = dest, weight = 'travTime')
        shortestPathTime = nx.shortest_path_length(graph, source = orig, target = dest, weight = 'travTime')
        return int(shortestPathTime), route
    ...
    def addRiderToFixedRoute(self, rider):
        newRoute = copy.copy(self.route)
        for i in range(len(newRoute)):
            if (newRoute[i][1] == rider.orig):
                newRoute.insert(i+1, (rider.rideID, rider.orig, 'pickup'))
                newRoute.insert(len(newRoute), (rider.rideID, rider.dest, 'dropoff'))
                return newRoute
    ...
    def calculateTravTime(self, startTime, route1, graph, timeDict):
        ...
        for i in range(len(route)-1):
            ...
            nextLoc = route[i+1][1]
            pathTime = nx.shortest_path_length(graph, source = currentLoc, target = nextLoc, weight = 'travTime')
            arrivalTime = departureTime + int(pathTime)
            # Determines the next node departure time based on next rider pickup and van arrival time and dwell time
            dwellTime = 30 if (route[i][1] != route[i+1][1]) else 0
            if (route[i+1][0] not in ['idle','stop']) and (arrivalTime < timeDict[route[i+1][0]]) and (route[i+1][2] == 'pickup'):
                departureTime = timeDict[route[i+1][0]] + dwellTime
            elif (route[i+1][0] not in ['idle','stop']) and (route[i+1][2] == 'dropoff'):
    ...
    def findBestRoute(self, graph, rider, timeDict, coeffWait, coeffDrive, uberRiders=None):
        ...
        for i in range(startIndex, len(newRoute)+1):
            ...
            #startIndex1 = i+1 if (self.fixedRoute == False) else len(newRoute)
